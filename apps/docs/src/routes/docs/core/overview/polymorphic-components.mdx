...
<Alert status="warning" isMultiline>
  <span>If you decide to change the default rendered node </span>
  <span>(e.g. change a `button` to a `div`) then it's up </span>
  <span>to you to ensure the correct accessibility </span>
  <span>and functionality of your chosen node type.</span>
</Alert>
